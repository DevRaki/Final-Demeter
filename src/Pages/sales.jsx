    ...
    if (newCategoryID === '') {
      ...
      if (selectedCategories.includes(newCategoryID)) {
        ...
        setSelectedCategoryName({
          ...
          [newCategoryID]: event.target.options[event.target.selectedIndex].text,
        });
      ...
      <div className="flex flex-grid">
        <div className="w-full md:w-[70%] lg:w-[60%] xl:w-[50%] min-h-screen md:min-h-[70%] lg:min-h-[80%] mx-auto md:mx-[25%] lg:mx-[30%] xl:mx-[35%] mr-5">
          ...
          {selectedCategories.map((categoryID, index) => (
            <div key={index}>
              <h1 className="text-3xl font-bold mb-4 mt-[10vh]">{selectedCategoryName[categoryID]}</h1>
              <div className="images flex flex-grid">
                {categoryImages[categoryID] &&
                  categoryImages[categoryID].map((image, imageIndex) => (
                    <button
                      key={imageIndex}
                      className="pl-[3vh] transform transition-transform hover:scale-105"
                      onClick={() => handleImageClick(categoryID, imageIndex)}
                    >
                      <img src={image} alt="" className="h-[3vh] cursor-pointer" />
